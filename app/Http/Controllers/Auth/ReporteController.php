...
class ReporteController extends Controller
{
    ...
    public function getRankingsByCategoryAndPlayer($filter_jugador_id,$filter_anio = 2024)
    {
        ...
        if ($Model != null) {
            $Rankings = Ranking::join('ranking_detalles', 'rankings.id', '=', 'ranking_detalles.ranking_id')
            ->where('rankings.comunidad_id', $Model->id)
            ->where(function ($query) use ($filter_jugador_id) {
                $query->where('ranking_detalles.jugador_simple_id', $filter_jugador_id)
                      ->orWhere('ranking_detalles.jugador_dupla_id', $filter_jugador_id);
            })
            ->select('rankings.*')
            ->get();
            ...
            $CategoriasTotal = Categoria::whereIn('id', array_values(array_unique(array_filter($TorneoCategorias->pluck('categoria_simple_id')->toArray()))))
                ->where('visible', true)->where('id', '!=', 3)->where('orden', '>', '0')
            ->orderBy('id', 'desc')->get();

            $RankingsResult = [];
            foreach ($CategoriasTotal as $q) {
                $Object = [];
                $JugadoresIds = [];
            ...
            foreach ($RankingsResult as $q2) {
                ...
                foreach ($jugadoresOrdenados as $key => $q3) {
                    ...
                    $result[] = [
                        ...
                        'anios' => $aniosData,
                        'id' => $q3['id'],
                        'categoria_id' => $q2->categoria_id,
                        'categoria_name' => $q2->categoria_name,
                    ];
                }
                $jugadoresOrdenadosGlobal[] = $result;
            }
            $filteredData = [];
   
            foreach ($jugadoresOrdenadosGlobal as $array) {
                foreach ($array as $item) {
                    if (isset($item['id']) && $item['id'] == $filter_jugador_id) {
                        $filteredData[] = $item;
                    }
                }
            }
