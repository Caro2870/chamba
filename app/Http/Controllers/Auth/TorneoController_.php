...
class TorneoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $Torneos = Torneo::where('comunidad_id', $Comunidad->id)->orderBy('fecha_final', 'desc')->pluck('fecha_inicio')->map(function ($q) {
            return Carbon::parse($q)->format('Y'); })->toArray();
    ...
    public function partialView($id)
    {
        ...
        if ($id != 0)
            $entity = Torneo::where('comunidad_id', Auth::guard('web')->user()->comunidad_id)->where('id', $id)->first();
    ...
    public function updateBackground(Request $request)
    {
        ...
        try {
            ...
            if ($request->file('imagen'))
                $imagen_path = $request->imagen->store('public/uploads/torneos/categorias');

            if ($request->id != 0)
                $entity = TorneoCategoria::where('id', $request->id)->first();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (!$Validator->fails()) {
                ...
                if ($request->file('imagen'))
                    $imagen_path = $request->imagen->store('public/uploads/torneos');

                if ($request->id != 0)
                    $entity = Torneo::where('comunidad_id', Auth::guard('web')->user()->comunidad_id)->where('id', $request->id)->first();
                ...
                if ($entity != null) {
                    ...
                    $ToneoCategorias = [];
                    $i = $entity->torneoCategorias()->orderBy('id', 'desc')->first() != null ? $entity->torneoCategorias()->orderBy('id', 'desc')->first()->sector + 1 : 1;
                    ...
                    if (count($Categorias) > 0) {
                        foreach ($Categorias as $q) {
                            if ($q->multiple) {
                                if (!in_array($q->categoria_simple_id . "-" . $q->categoria_dupla_id, $request->categorias))
                                    $entity->torneoCategorias()->where('id', $q->id)->update(['user_update_id' => Auth::guard('web')->user()->id, 'deleted_at' => Carbon::now()]);
                            } else {
                                if (!in_array($q->categoria_simple_id, $request->categorias))
                                    $entity->torneoCategorias()->where('id', $q->id)->update(['user_update_id' => Auth::guard('web')->user()->id, 'deleted_at' => Carbon::now()]);
                            }
                        }

                        $Categorias = $Categorias->map(function ($obj, $key) use ($request) {
                            return $obj->multiple ? ($obj->categoria_simple_id . "-" . $obj->categoria_dupla_id) : $obj->categoria_simple_id; })->toArray();

                        foreach ($request->categorias as $q) {
                            if (!in_array($q, $Categorias)) {
                                $categorias = explode("-", $q);
                                if (count($categorias) == 1) {
                                    $ToneoCategorias[] = ['torneo_id' => $entity->id, 'categoria_simple_id' => $categorias[0], 'categoria_dupla_id' => null, 'sector' => $i, 'multiple' => false, 'user_create_id' => Auth::guard('web')->user()->id];
                                } else {
                                    $ToneoCategorias[] = ['torneo_id' => $entity->id, 'categoria_simple_id' => $categorias[0], 'categoria_dupla_id' => $categorias[1], 'sector' => $i, 'multiple' => true, 'user_create_id' => Auth::guard('web')->user()->id];
                                }
                                $i++;
                            }
                        }
                    ...
                    if ($request->categorias != null) {
                        foreach ($request->categorias as $key => $q) {
                            $categorias = explode("-", $q);
                            if (count($categorias) == 1) {
                                $ToneoCategorias[] = ['torneo_id' => $entity->id, 'categoria_simple_id' => $categorias[0], 'categoria_dupla_id' => null, 'orden' => $key, 'sector' => $i, 'multiple' => false, 'user_create_id' => Auth::guard('web')->user()->id];
                            } else {
                                $ToneoCategorias[] = ['torneo_id' => $entity->id, 'categoria_simple_id' => $categorias[0], 'categoria_dupla_id' => $categorias[1], 'orden' => $key, 'sector' => $i, 'multiple' => true, 'user_create_id' => Auth::guard('web')->user()->id];
                            }
                            $i++;
                        }
                    }
                ...
                if (count($ToneoCategorias) > 0)
                    TorneoCategoria::insert($ToneoCategorias);

                if (count($request->zonas_id) > 0) {
                    foreach ($request->zonas_id as $q)
                        TorneoZona::create(['torneo_id' => $entity->id, 'zona_id' => $q]);
                }
            ...
            if ($TorneoCategoria != null) {
                ...
                if ($entity->save() && $TorneoCategoria->save()) {
                    ...
                    if ($Rankings != null && count($Rankings) > 0) {
                        ...
                        if (count($RankingDetalle) > 0)
                            RankingDetalle::insert($RankingDetalle);
                    }
    ...
    public function categorias(Request $request)
    {
        $Categorias = TorneoCategoria::where('torneo_id', $request->torneo)
            ...
            ->get()->pluck('categoriaSimple')->map(function ($q) {
                return ['id' => $q->id, 'nombre' => $q->nombre]; });
    ...
    public function categoriaStore(Request $request)
    {
        ...
        try {
            ...
            if ($request->clasificados != 3)
                $TorneoCategoria->clasificados_terceros = 0;
            else {
            ...
            if ($Clasifican == 3) {
                ...
                foreach (collect($JugadoresClasificados)->pluck('Clasificados') as $key => $value) {
                    foreach ($value as $ke2 => $value2) {
                        if (!in_array($value2['key'], $Clasificados))
                            $SegundoLugares[] = $value2;
                    }
                }
    ...
    public function faseFinalStore(Request $request)
    {
        ...
        try {
            ...
            if ($TorneoCategoria != null) {
                ...
                if (
                    !$reload && count($TorneoCategoria->torneo->partidos->where('torneo_categoria_id', $TorneoCategoria->id)
                ...
                if (!in_array(count($JugadoresClasificadosMerge), [4, 8, 16, 32])) {
                    $BuyGenerar = 0;
                    if (count($JugadoresClasificadosMerge) < 16)
                        $BuyGenerar = (16 - count($JugadoresClasificadosMerge));
                    else if (count($JugadoresClasificadosMerge) < 32)
                        $BuyGenerar = (32 - count($JugadoresClasificadosMerge));
                        ...
                        foreach ($JugadoresNoDefinidos as $q) {
                            if (!in_array($q['key'], $JugadoresKeysDefinidos)) {
                                ...
                                if ($JugadorRival != null) {
                                    $JugadoresKeysDefinidos[] = $q['key'];
                                    $JugadoresKeysDefinidos[] = $JugadorRival['key'];
                                    $PartidosFaseFinal[] = [
                                        'buy' => false,
                ...
                if ($request->tipo == "random")
                    $PartidosFaseFinal = collect($PartidosFaseFinal)->shuffle();
                ...
                while ($i <= 100) {
                    ...
                    if (count(collect($PartidosFaseFinal)->whereNotIn('key', $PartidosKeysDefinidos)) == 0) {
                        break;
                    } else {
                        $i++;
                    }
                ...
                $BloquesFaseFinal = [$PrimerBloque, $SegundoBloque, $TercerBloque, $CuartoBloque];
                $Partidos = [];
                ...
                foreach ($BloquesFaseFinal as $key => $bloque) {
                    ...
                    foreach ($bloque as $q) {
                        $Partidos[] = [
                            ...
                            'fecha_inicio' => $fechaInicioPartido,
                            'fecha_final' => $fechaFinalPartido,
                            'hora_inicio' => null,
                            'hora_final' => null,
                            'resultado' => null,
                            ...
                            switch (count($q)) {
                                case 1:
                                    $Partidos[] = [
                                        ...
                                        'fase' => count($PartidosFaseFinal) / 2,
                                        'user_create_id' => Auth::guard('web')->user()->id,
                                    ];
                                    break;
                                case 2:
            ...
            if (!$TorneoCategoria->first_final) {
                ...
                $Partidos = Partido::where('torneo_categoria_id', $request->torneo_categoria_id)->where('torneo_id', $request->torneo_id)
                    ->whereHas('torneo', function ($q) {
                        $q->where('comunidad_id', Auth::guard('web')->user()->comunidad_id); })
                    ->where(function ($q) use ($request) {
                        if ($request->partido_id != null && intval($request->partido_id) > 0)
                            $q->where('id', '!=', $request->partido_id);
                ...
                if ($TorneoCategoria->multiple) {
                    foreach ($Partidos as $q) {
                        if ($q->jugador_local_uno_id != null && $q->jugador_local_dos_id)
                            $JugadoresNoDisponibles[] = $q->jugador_local_uno_id . "-" . $q->jugador_local_dos_id;
                        if ($q->jugador_rival_uno_id != null && $q->jugador_rival_dos_id)
                            $JugadoresNoDisponibles[] = $q->jugador_rival_uno_id . "-" . $q->jugador_rival_dos_id;
                    }
                } else {
                ...
                $JugadoresClasificados = $JugadoresClasificadosMerge->where('key', '!=', $request->jugador_selected_id)
                    ...
                    ->filter(function ($q) use ($request) {
                        return str_contains(App::Unaccent(strtolower($q['nombres'])), App::Unaccent(strtolower($request->nombre))); })
                    ->map(function ($q) {
                        return ['id' => $q['key'], 'text' => $q['nombres']]; })->toArray();
    ...
    public function faseFinalPrePartidoStore(Request $request)
    {
        ...
        try {
            ...
            if (!$Validator->fails()) {
                ...
                if ($entity != null) {
                    $JugadorLocalUno = null;
                    $JugadorLocalDos = null;
                    $JugadorRivalUno = null;
                    $JugadorRivalDos = null;
                    ...
                    if (!$entity->buy && !$entity->buy_all) {
                        $Result->Repeat = Partido::where('torneo_categoria_id', $entity->torneo_categoria_id)
                            ...
                            ->where(function ($q) use ($JugadorLocalUno, $JugadorRivalUno) {
                                $q->where(function ($q2) use ($JugadorLocalUno, $JugadorRivalUno) {
                                    $q2->where('jugador_local_uno_id', $JugadorLocalUno)->where('jugador_rival_uno_id', $JugadorRivalUno); })
                                    ->orWhere(function ($q2) use ($JugadorLocalUno, $JugadorRivalUno) {
                                        $q2->where('jugador_local_uno_id', $JugadorRivalUno)->where('jugador_rival_uno_id', $JugadorLocalUno); });
                            })->first();
                    }
    ...
    public function faseFinalPrePartidoPartialView($torneo_id, $torneo_categoria_id, $id, $position, $bracket)
    {
        ...
        if ($TorneoCategoria != null)
            return view('auth' . '.' . $this->viewName . '.ajax.final.prepartido.partialView', ['TorneoCategoria' => $TorneoCategoria, 'Partido' => $Partido, 'Position' => $position, 'Bracket' => $bracket, 'ViewName' => ucfirst($this->viewName)]);
                ...
                if ($Grupo != null) {
                    ...
                    foreach ($Jugadores as $key2 => $q2) {
                        if ($TorneoCategoria->multiple) {
                            ...
                            foreach ($PartidosComoLocal as $p) {
                                if ($p->estado_id == App::$ESTADO_FINALIZADO)
                                    $PartidosJugados += 1;
                        ...
                        if ($Partido != null) {
                            ...
                            if ($Partido->save()) {
                                if ($Partido->estado_id == App::$ESTADO_FINALIZADO && $Partido->fase > 1) {
                                    ...
                                    if ($PartidoNext->buy)
                                        $PartidoNextWiouthBuy = $PartidoNext;
    ...
    public function obtenerLlave($LlavesRestantes, $GrupoLocal, $GrupoRival)
    {
        ...
        if ($Seleccionado == null) {
            ...
            if (count($Seleccionado) > 0)
                $Seleccionado = $Seleccionado->random();
            else
                $Seleccionado = collect($LlavesRestantes)->first();
        }
    ...
    public function grupo($id, $torneo_categoria_id = null, $fase = null, $landing = false)
    {
        ...
        if ($id != 0) {
            ...
            if ($torneo_categoria_id == null || $torneo_categoria_id == 0)
                $torneo_categoria_id = $entity->torneoCategorias()->where('torneo_id', $id)->orderBy('orden')->first()->id;
        }
    ...
    public function grupoPartialView($torneo_id, $categoria_id, $tipo = null)
    {
        ...
        $Jugadores = TorneoJugador::whereHas(
            'torneo',
            function ($q) {
                $q->where('comunidad_id', Auth::guard('web')->user()->comunidad_id); }
        )
        ...
        return view(
            'auth' . '.' . $this->viewName . '.ajax.grupo.partialView',
    ...
    public function grupoTablaPartialView($id, $torneo_categoria_id, $grupo_id, $landing = false)
    {
        ...
        $TorneoCategoria = TorneoCategoria::where('id', $torneo_categoria_id)->where('torneo_id', $id)
            ->whereHas('torneo', function ($q) use ($ComunidadId) {
                $q->where('comunidad_id', $ComunidadId); })->first();
        ...
        if ($TorneoCategoria != null) {
            ...
            return view('auth' . '.' . $this->viewName . '.ajax.grupo.tabla.partialView', [
                'TablaPosiciones' => App::multiPropertySort(collect($TablePositions), [
                    ['column' => 'puntos', 'order' => 'desc'],
                    ['column' => 'setsDiferencias', 'order' => 'desc'],
                    ['column' => 'gamesDiferencias', 'order' => 'desc'],
                    ['column' => 'setsGanados', 'order' => 'desc'],
                    ...
                    if (count($Torneo->partidos->where('torneo_categoria_id', $request->torneo_categoria_id)) <= 0) {
                        ...
                        if ((count($Jugadores) % 4) == 0) {
                            ...
                            foreach ($OriginalOrder as $q) {
                                foreach ($RandomOrder as $key2 => $q2) {
                                    if ($q == $q2)
                                        $Positions[] = $key2;
                                }
                            }
                                    ...
                                    if (count($PartidosModel) > 0) {
                                        ...
                                        for ($i = 0; $i < ($CantidadPartidos / 2); $i++) {
                                            ...
                                            foreach ($PartidosModelCollect->whereNotIn('key', collect($PartidosModelWithDates)->pluck('key')->toArray()) as $q2) {
                                                if (
                                                    !in_array($FirstRoundMatchOne->JugadorLocal, [$q2->JugadorLocal, $q2->JugadorRival]) &&
    ...
    public function grupoManualPartialView($torneo_id, $categoria_id, $tipo = null)
    {
        ...
        return view(
            'auth' . '.' . $this->viewName . '.ajax.grupo.manual.partialView',
                    ...
                    if (count($Torneo->partidos->where('torneo_categoria_id', $request->torneo_categoria_id)) <= 0 || $request->agregar) {
                        ...
                        if ((count($Jugadores) % 4) == 0) {
                            if ((count($request->grupo_id == null ? [] : $request->grupo_id) * 4) != (count($request->jugador_uno_id == null ? [] : $request->jugador_uno_id) + count($request->jugador_dos_id == null ? [] : $request->jugador_dos_id) + count($request->jugador_tres_id == null ? [] : $request->jugador_tres_id) + count($request->jugador_cuatro_id == null ? [] : $request->jugador_cuatro_id))) {
                                ...
                                $LasPositionGrupo = 0;
                                if ($request->agregar) {
                                    $TorneoGruposExistentes = TorneoGrupo::whereHas(
                                        'torneo',
                                        function ($q) {
                                            $q->where('comunidad_id', Auth::guard('web')->user()->comunidad_id); }
                                    )
                                ...
                                for ($i = 0; $i < 4; $i++) {
                                    ...
                                    if ($TorneoCategoria->multiple) {
                                        for ($i2 = 0; $i2 < count($jugador_id); $i2++) {
                                            ...
                                            if ($result != null)
                                                $TorneoCategoriaEscogidos[] = $result;
                                        }
    ...
    public function grupoAgregarPartialView($torneo_id, $categoria_id)
    {
        ...
        if ((count($Jugadores) % 4) == 0) {
            $TorneoGrupos = TorneoGrupo::whereHas(
                'torneo',
                function ($q) {
                    $q->where('comunidad_id', Auth::guard('web')->user()->comunidad_id); }
            )
        ...
        return view(
            'auth' . '.' . $this->viewName . '.ajax.grupo.agregar.partialView',
    ...
    public function jugadorListJson(Request $request)
    {
        ...
        $TorneoGrupo = TorneoGrupo::whereHas('torneo', function ($q) {
            $q->where('comunidad_id', Auth::guard('web')->user()->comunidad_id); })
            ...
            if ($TorneoCategoria->multiple) {
                ...
                $Jugadores = $Torneo->torneoJugadors()->where('torneo_categoria_id', $request->torneo_categoria_id)
                    ...
                    ->where(function ($q) use ($request) {
                        $q->whereHas('jugadorSimple', function ($q) use ($request) {
                            if ($request->nombre) {
                                $q->where(DB::raw("CONCAT(jugadors.nombres,' ',jugadors.apellidos)"), 'like', '%' . $request->nombre . '%');
                            }
                    ...
                    ->get()
                    ->map(function ($q) {
                        return ['id' => $q->id, 'text' => ($q->jugadorSimple->nombre_completo . ' + ' . $q->jugadorDupla->nombre_completo)]; });
                ...
                $Jugadores = $Torneo->torneoJugadors()->where('torneo_categoria_id', $request->torneo_categoria_id)
                    ...
                    ->whereHas('jugadorSimple', function ($q) use ($request) {
                        ...
                        return ['id' => $q->jugador_simple_id, 'text' => $q->jugadorSimple->nombre_completo]; });
            }
        }
    ...
    public function jugadorZonaStore(Request $request)
    {
        ...
        try {
            ...
            if (!$Validator->fails()) {
                ...
                if ($request->id != 0)
                    $entity = TorneoJugador::find($request->id);
        ...
        try {
            ...
            if (!$Validator->fails()) {
                ...
                if ($TorneoCategoria->multiple) {
                    ...
                    $Update = 0;
                    if ($request->jugador_remplazo_id != null && $request->jugador_remplazo_dupla_id == null)
                        $Update = 1;
                    else if ($request->jugador_remplazo_id == null && $request->jugador_remplazo_dupla_id != null)
                        $Update = 2;
                    else if ($request->jugador_remplazo_id != null && $request->jugador_remplazo_dupla_id != null)
                        $Update = 3;
    ...
    public function grupoPartidoStore(Request $request)
    {
        ...
        try {
            ...
            $request->merge([
                'comunidad_id',
                Auth::guard('web')->user()->comunidad_id,
                'fecha_actual' => Carbon::now()->toDateString(),
            ]);
        ...
        if ($TorneoCategoria != null) {
            ...
            if ($Ranking != null) {
                ...
                $TablePositions = App::multiPropertySort(collect($TablePositions), [
                    ['column' => 'partidosJugados', 'order' => 'desc'],
                    ['column' => 'puntos', 'order' => 'desc'],
                    ['column' => 'setsDiferencias', 'order' => 'desc'],
                    ['column' => 'gamesDiferencias', 'order' => 'desc'],
                    ['column' => 'setsGanados', 'order' => 'desc'],
